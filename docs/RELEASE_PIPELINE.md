...
- `apple`: full Xcode 27.0, iOS 27 SDK and simulator, iPhone Release build, unit tests, deterministic checkout/Arabic/account-deletion/offline/Bluetooth UI journeys, an eight-size iPhone/iPad layout matrix, Watch Release build, iPhone widget build, and Watch widget build. The app targets iOS 17 and watchOS 10; iOS/watchOS 26-only presentation enhancements remain availability-gated. See [device optimization](IOS_27_DEVICE_OPTIMIZATION.md) for the matrix and verification limits.
...
External alert delivery is a deployment gate, not a local default: this repository deliberately contains no webhook value. After setting `UPTIME_ALERT_WEBHOOK_URL` in GitHub, manually dispatch **External uptime**, enable **Send a test notification**, and verify that the labeled message arrives. Configure `OPS_ALERT_WEBHOOK_URL` in Render and exercise a staging 5xx to verify the backend error path. The manual delivery test fails closed when its protected webhook is absent.
