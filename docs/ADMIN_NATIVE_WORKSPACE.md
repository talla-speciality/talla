...
The admin app now has five tabs: Orders, Products, Customers, Admin, and Settings. The Admin tab opens native SwiftUI screens for App Home, Live Controls, Seasonal Events, Coffee Passport, Notifications, Analytics, Operations, Taste Memory, and Recent Admin Actions. App Home includes English and Arabic hero controls. Live Controls includes an independent Click to Pay switch. The old web console is no longer routed from the app.
...
- CI Release simulator build for the Talla Admin scheme.
